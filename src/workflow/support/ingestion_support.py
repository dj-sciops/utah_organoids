    ...
    def make(self, key):
        ...
        if Path(REL_PATH_INBOX) in remote_fullpath.parents:
            ...
            if remote_fullpath.suffix == ".rhs":
                ...
                file_times = (
                    ephys.EphysRawFile & {"parent_folder": parent_dir.name}
                ).fetch("file_time", order_by="file_time")
                ...
                session_entries.append((start, previous))
                log_message += (
                    f"Creating {len(session_entries)} ephys session(s) for {subject_key}: {session_entries}"
                    + "\n"
                )
